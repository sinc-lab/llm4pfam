...
###############################################################################
desc="Precompute protTrans ESM1v to speed up training."
...
for m in range(1,6):
    ...
    for item in tqdm(range(len(data))):
        ...
        if args.premb:
            ...
            if m>1:
                prev_emb, _ = pickle.load(open(out_file, "rb"))
                emb+=prev_emb
            if m==5:
                emb/=5.0
            # If m==1, save file for first time, else save accumulation.
            # If m==5, save average
